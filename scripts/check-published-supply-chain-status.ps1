...
Require-Text $evidence "Build/package provenance | VERIFIED PUBLISHED STATE / HOSTED PASS" "Provenance published-state evidence"
Require-Text $evidence "35295200" "RC1 nupkg attestation evidence"
Require-Text $evidence "35295205" "RC1 snupkg attestation evidence"
...
Require-Text $decisions '**Decision: IMPLEMENTED AND HOSTED-VERIFIED FOR `1.0.0-rc.1`.**' "Hosted provenance decision"
...
if (Get-Command git -ErrorAction SilentlyContinue) {
    ...
    try {
        $workingTree = Get-GitWorkingTreeStatus
        if ($workingTree.ExitCode -eq 0 -and $workingTree.Lines) {
