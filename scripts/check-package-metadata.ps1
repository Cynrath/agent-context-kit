...
function Require-Value {
    param([xml]$Project, [string]$Name, [string]$Expected)
    $actual = Get-MetadataValue -Project $Project -Name $Name
    if ($actual -ne $Expected) { Add-Issue "$Name expected '$Expected' but found '$actual'." }
}

function Test-PackedReadme {
    param([xml]$Project)
    foreach ($node in $Project.SelectNodes("//None")) {
        if ($node.GetAttribute("Include") -eq "..\..\README.nuget.md" -and $node.GetAttribute("Pack") -eq "true" -and $node.GetAttribute("PackagePath") -eq "\") {
            return $true
        }
    }
    return $false
...
if (-not (Test-Path $projectPath)) {
    ...
    Require-Value -Project $project -Name "PackageReadmeFile" -Expected $readmeFile
    ...
    if ([string]::IsNullOrWhiteSpace($version) -or $version -notmatch "^\d+\.\d+\.\d+([-.][0-9A-Za-z.-]+)?$") { Add-Issue "Version is missing or does not look like a SemVer package version." }

    $description = Get-MetadataValue -Project $project -Name "Description"
    if ([string]::IsNullOrWhiteSpace($description) -or $description.Length -lt 20) { Add-Issue "Description is missing or too short." }

    $tags = Get-MetadataValue -Project $project -Name "PackageTags"
    foreach ($requiredTag in @("ai", "coding-agent", "security", "cli", "oss")) {
        $tagValues = @()
        if (-not [string]::IsNullOrWhiteSpace($tags)) { $tagValues = $tags -split "[;\s]+" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) } }
        if ($tagValues -notcontains $requiredTag) { Add-Issue "PackageTags does not include '$requiredTag'." }
    }

    if ([string]::IsNullOrWhiteSpace((Get-MetadataValue -Project $project -Name "PackageReleaseNotes"))) { Add-Issue "PackageReleaseNotes is missing." }

    foreach ($urlField in @("RepositoryUrl", "PackageProjectUrl")) {
        $url = Get-MetadataValue -Project $project -Name $urlField
        if ([string]::IsNullOrWhiteSpace($url) -or $url -match "TODO|example\.com|localhost") { Add-Issue "$urlField is missing or still a placeholder." }
    }
...
Write-Host ""
if ($issues.Count -eq 0) { Write-Host "No package metadata issues detected." }
else {
...
Write-Host ""
Write-Host "This review is local-only and report-only unless -FailOnIssues is passed."
