...
Status date: 2026-06-20. Decision boundary: local `0.2.0-alpha.3` release candidate prepared by TASK-0203; publication remains unapproved until hosted RC evidence and a dedicated exact-candidate GO are recorded.
...
| RB-009 | Candidate version | P0/P1 dependent | Local candidate prepared: `0.2.0-alpha.3` at implementation commit `33e1897`; local pack/verify/install smoke passed; hosted RC evidence pending | Release maintainer | Scope, compatibility, package diff, changelog, exact commit, prerequisite disposition | Yes in next task | Obtain hosted RC evidence; do not publish from release preparation | After hosted RC evidence | Hosted evidence pending | TASK-0203 local package evidence recorded; hosted RC evidence still required |
| RB-010 | Release approval | P0 | Publish GO not recorded | Release maintainer | Completed checklist, local/hosted evidence, blocker dispositions, package/release review | Yes later | GO or NO-GO for exact candidate; no implicit publication approval | After hosted RC evidence | Blocks publish, tag, GitHub Release, NuGet publish, and workflow dispatch | New signed-off GO decision for exact commit/version |
