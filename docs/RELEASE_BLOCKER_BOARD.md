...
Status date: 2026-06-20. Decision boundary: release-preparation eligible for `0.2.0-alpha.3`; publication remains unapproved until a dedicated preparation/validation task records an exact-candidate GO.
...
| RB-009 | Candidate version | P0/P1 dependent | Scope selected: `0.2.0-alpha.3`; blockers closed; TASK-0203 prepares source/package metadata for local validation; exact validated candidate evidence pending | Release maintainer | Scope, compatibility, package diff, changelog, exact commit, prerequisite disposition | Yes in next task | Complete TASK-0203 local pack/install evidence; do not publish from release preparation | After TASK-0203 validation | Local candidate preparation | Dedicated approved release-preparation task with local package evidence |
| RB-010 | Release approval | P0 | Release-preparation in progress; publish GO not recorded | Release maintainer | Completed checklist, local/hosted evidence, blocker dispositions, package/release review | Yes later | GO or NO-GO for exact candidate; no implicit publication approval | After local validation and hosted RC evidence | Blocks publish, tag, GitHub Release, NuGet publish, and workflow dispatch | New signed-off GO decision for exact commit/version |
