...
| RB-009 | Candidate version | P0/P1 dependent | Scope selected: `0.2.0-alpha.3`; blockers closed; metadata and exact commit not prepared | Release maintainer | Scope, compatibility, package diff, changelog, exact commit, prerequisite disposition | Yes in next task | Proceed through dedicated release-preparation task; do not publish from TASK-0202 | Before metadata change | Next required task | Dedicated approved release-preparation task |
| RB-010 | Release approval | P0 | Release-preparation eligible; publish GO not recorded | Release maintainer | Completed checklist, local/hosted evidence, blocker dispositions, package/release review | Yes later | GO or NO-GO for exact candidate; no implicit publication approval | After release-preparation validation | Blocks publish, tag, GitHub Release, NuGet publish, and workflow dispatch | New signed-off GO decision for exact commit/version |
