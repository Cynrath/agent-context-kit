...
| V100-09 | P1 | Release supply-chain policy still lacks hosted provenance evidence. | TASK-0242 published NuGet RC1 without tag/release/provenance. TASK-0244 and TASK-0247 failed before mutation. TASK-0249 fixed expected-404 handling and passed three-OS CI. TASK-0250 run `29341087462` passed all pre-mutation gates, but GitHub rejected the App token's tag push for missing `workflows` permission. One audit confirmed NuGet/artifact unchanged and tag/release/assets/two attestations absent. | Release + Maintainer | Preserve every partial history. A new explicit decision is required; this task cannot change settings, retry, rerun, redispatch, or complete the release manually. | Yes | Yes, but the TASK-0250 budget is consumed | THIRD_RECOVERY_ATTEMPT_STOPPED / TAG_PUSH_REJECTED / REMOTE_STATE_UNCHANGED / OPEN_PENDING_NEW_DECISION |
