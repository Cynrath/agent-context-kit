...
| V100-09 | P1 | Release supply-chain policy still lacks hosted provenance evidence. | TASK-0242 published NuGet RC1 without tag/release/provenance. TASK-0243 implemented exact recovery; TASK-0244 run `29151228607` failed in a pre-mutation Ubuntu safety gate because a fixture helper called Windows-only `powershell`. One-time audit confirmed tag/release/two attestations absent and package evidence unchanged. | Release + Maintainer | Preserve both partial histories. A new explicit decision is required before fixing the hosted-only script issue or authorizing any future recovery attempt. | Yes | Yes, but the TASK-0244 budget is consumed | RECOVERY_ATTEMPT_STOPPED / NO_REMOTE_MUTATION / TAG_RELEASE_ATTESTATIONS_ABSENT / OPEN_PENDING_NEW_DECISION |
...
Open P0 gaps: 0. V100-06 remains closed by TASK-0232; TASK-0241 closes V100-01 through V100-05 from exact local/hosted evidence and final acceptance. V100-09 remains open after the single TASK-0244 recovery attempt stopped before mutation; any script correction or future recovery needs a new explicit decision.
