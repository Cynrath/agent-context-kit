...
| V100-09 | P1 | Release supply-chain policy still lacks hosted provenance evidence. | TASK-0255 reverified the immutable source/NuGet/tag tuple and created exact prerelease `353913024` with prepared body and only exact assets `476881883`/`476881892`; sizes, API digests, and downloaded hashes pass. NuGet/tag remained unchanged. Both attestations and Windows/Ubuntu/macOS installed-package evidence remain pending TASK-0256. | Release + Maintainer | Preserve every partial history; complete minimal attestation-only verification and three-platform smoke before public/V100 closure. | Yes | Exact release write completed; attestation dispatch pending | EXACT_RELEASE_ASSETS_PASS / ATTESTATIONS_AND_THREE_PLATFORM_PENDING / OPEN |
