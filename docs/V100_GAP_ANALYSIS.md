...
| V100-09 | P1 | Release supply-chain policy still lacks hosted provenance evidence. | TASK-0242 published NuGet RC1 without tag/release/provenance. TASK-0244 failed in pre-mutation safety gates. TASK-0246 fixed `pwsh` portability and passed three-OS standard CI. TASK-0247 run `29182188201` passed exact artifact/package/signature/install validation, then exited after the expected absent-release probe before mutation. One-time audit again confirmed tag/release/two attestations absent and package evidence unchanged. | Release + Maintainer | Preserve all partial histories. A new explicit decision is required before correcting the workflow exit-state issue or authorizing any future recovery attempt. | Yes | Yes, but the TASK-0247 budget is consumed | SECOND_RECOVERY_ATTEMPT_STOPPED / NO_REMOTE_MUTATION / TAG_RELEASE_ATTESTATIONS_ABSENT / OPEN_PENDING_NEW_DECISION |
