...
| V100-09 | P1 | Release supply-chain policy still lacks hosted provenance evidence. | OIDC/immutability/alignment/inspection/digest/signature/asset/upgrade/rollback/recovery baseline and backup recovery ownership are accepted; signing/SBOM dated risks remain active. TASK-0240 is not a publish path and proves no attestation. | Release + Maintainer | Future TASK-0242 must publish through authorized OIDC, create and verify exact package/release attestation, and record provenance. | Yes | Yes, only in separately authorized publish task | MAINTAINER_DECISION_RECORDED / RECOVERY_OWNERSHIP_RECONCILED / SIGNING_AND_SBOM_ACCEPTED_RISK_ACTIVE / OPEN_PENDING_PUBLISH_PATH_PROVENANCE |
