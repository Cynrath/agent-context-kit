...
The current published `1.0.0-rc.1` package is available through NuGet.org, repository-signed, and linked to exact repository commit `258918b33c3d1359aac967604ee524e8b66ddf02`. Exact GitHub nupkg/snupkg assets and attestations are verified by TASK-0255/TASK-0256. Author signing and SBOM publication remain explicitly unclaimed bounded risks. Exact current and historical NuGet/release hashes plus recovery evidence remain in `docs/PUBLISHED_SUPPLY_CHAIN_STATUS.md`.
...
For the completed `1.0.0-rc.1` recovery, the package, exact tag, release body, and assets are immutable inputs. The final `attest-existing` path contains no NuGet publication, tag mutation, or release creation; it verifies the exact release tuple before attesting, verifies both attestations, rechecks immutability, and requires Windows/Ubuntu/macOS installed-package smoke. Historical failed recovery dispatches remain preserved.
