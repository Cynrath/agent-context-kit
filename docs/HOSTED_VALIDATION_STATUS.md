...
TASK-0256 implemented the `attest-existing` operation: exact release/body/assets and repository-signed NuGet equivalence precede two conditional attestations; both CLI verifications and final immutable recheck are mandatory; a dependent Windows/Ubuntu/macOS installed-package matrix follows. Local validation passed 431/431 tests, all focused/static gates, YAML parsing, ACKit, and Unicode 0/0. Hosted evidence completed in corrected run `29350091782` as recorded below.
