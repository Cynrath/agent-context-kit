...
TASK-0253 may run exactly one NuGet-publish-free recovery dispatch only after TASK-0252 exact-HEAD standard CI is green. Any remote failure consumes that authorization and triggers one log read, one remote audit, factual documentation, and safe stop without rerun or manual completion. TASK-0254 remains success-only and may update the published smoke pin/public evidence/V100-09 only after the prerelease, exact two assets, both verified attestations, and Windows/Ubuntu/macOS installed-tool recovery jobs all pass.
