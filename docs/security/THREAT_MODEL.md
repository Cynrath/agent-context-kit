...
| T18 | Forged verifier verdict | Verifier authors `ackit.verdict.v1`; ACKit persists state-bound `ackit.verdict.v2` (binding + reviewed-bundle reference); registration validates task existence, criterion references, and blocking-finding/verdict consistency; append-only store (a REWORK verdict cannot be overwritten, only superseded by a later registered verdict) | TASK-0052 verdict validation matrix + TASK-0060 forgery tests |
