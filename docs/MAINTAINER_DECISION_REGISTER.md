...
| MD-010 | RB-010 approval | Release-preparation eligible / publish deferred | Prepare `0.2.0-alpha.3` in a dedicated task; do not publish yet | TASK-0202 closes ownership/recovery blockers, but exact candidate metadata/package/hosted evidence/GO do not exist yet | `Cynrath` release maintainer | `docs/V020_ALPHA3_RELEASE_DECISION.md`; TASK-0202 maintainer evidence; current metadata remains `0.2.0-alpha.2` | Planned `0.2.0-alpha.3`; no candidate commit | Until release preparation records exact-candidate GO or NO-GO | Documentation push only in TASK-0202; no release write |
