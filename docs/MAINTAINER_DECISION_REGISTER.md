...
| MD-025 | TASK-0252 existing exact-tag adaptation | Complete / pushed / CI pass | Verify the owner-created exact tag fail-closed and remove every recovery tag mutation | Exact tag removes the failed App tag-push dependency while preserving immutable package identity | Cynrath release maintainer | Commit `5f6c4ce`; runs `29344903472`, `29344903420`, `29344903850` | Recovery automation only; no release mutation | Reopen after workflow/helper safety change | Normal push only; no recovery dispatch or immutable release mutation |
| MD-026 | TASK-0253 exact-tag recovery | Consumed / release create 403 / stopped | Single run `29345313517` passed immutable gates; GitHub returned HTTP 403 at prerelease creation | User authorized exactly one dispatch and required log-once/audit-once safe stop on failure | Cynrath release maintainer | Run/job `29345313517`/`87127346868`; one failed-log read; one immutable audit | NuGet/artifact/exact tag unchanged; prerelease/assets/two attestations absent; matrix skipped; smoke pin alpha4; V100-09 open | Dispatch consumed; TASK-0254 not executed; retry/manual/settings paths prohibited | No NuGet push, tag mutation, release/asset/attestation creation, rerun, second dispatch, manual completion, settings change, or force push |
| MD-027 | TASK-0255 exact authenticated release completion | Complete / exact release and assets verified | Diagnose Actions integration 403 and use existing ADMIN local `gh` identity to create only the exact existing-tag prerelease and retained TASK-0242 assets | Current full authorization permits release completion while NuGet/tag remain immutable; local identity has repository write/admin and exact inputs were reverified | Cynrath release maintainer | Release `353913024`; asset IDs `476881883`/`476881892`; exact body/target/size/API-digest/download-hash evidence | GitHub prerelease/body/exact two assets for RC1 | Attestations and three-platform proof pending TASK-0256 | One release creation; zero NuGet publish, tag mutation, settings/PAT/secret mutation, force push, or history rewrite |
