...
Config allowlist fields are part of the current source and published `1.0.0-rc.1` package. The published NuGet `1.0.0-rc.1` package includes the scanner rule catalog and allowlist feature set.
...
The published `1.0.0-rc.1` package records configured non-Critical suppressions in local `ackit scan` human/JSON output. Audit records identify the rule, severity, category, repository-relative path, and config reason without including raw matches. See [SUPPRESSION_AUDIT.md](SUPPRESSION_AUDIT.md).
