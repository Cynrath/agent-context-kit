...
Config allowlist fields are part of the current source and published `0.2.0-alpha.4` package. The published NuGet `0.2.0-alpha.4` package includes the scanner rule catalog and allowlist feature set.
...
The published `0.2.0-alpha.4` package records configured non-Critical suppressions in local `ackit scan` human/JSON output. Audit records identify the rule, severity, category, repository-relative path, and config reason without including raw matches. See [SUPPRESSION_AUDIT.md](SUPPRESSION_AUDIT.md).
