...
TASK-0246 through TASK-0248 are a separate newly authorized continuation. TASK-0246 resolves and invokes `pwsh`, rejects Windows PowerShell regression, verifies the safety/artifact/state gates remain before tag/release/attestation mutation, and runs those tests in the Windows/Ubuntu/macOS source-smoke matrix. After its standard CI is fully green, TASK-0247 may dispatch exact recovery exactly once. TASK-0248 remains conditional on complete recovery success. This new decision does not permit normal publish, NuGet mutation, workflow rerun, a second TASK-0247 dispatch, manual upload, tag movement, force update, or GA readiness claims.
