...
TASK-0187 re-audited all three visible `nuget-release` failed deployment entries: release #4 job `81200598792` failed on a Windows-only `powershell` command in Ubuntu, while release #5 jobs `81201079722` and `81201198341` failed on published-package temp path resolution. A delayed release #4 failed-job rerun failed at artifact download because the validated package artifact had expired after one day. Current source guards these classes: the release workflow static gate rejects `powershell` inside the Ubuntu publish job, the published/existing release verifiers use lazy writable temp resolution with local self-test coverage, and the validated package artifact retention is 14 days. This did not mutate historical deployment records.
