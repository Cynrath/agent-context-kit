...
TASK-0244 alone could dispatch `recover-existing`, exactly once and only after TASK-0243 push-triggered CI was green. That budget was consumed by run `29151228607`. The run failed in the Ubuntu safety-gate step because `scripts/test-supply-chain-workflow.ps1` called Windows-only `powershell`; all remote mutation and three-platform verification steps were skipped. The failed log was read once and one immutable audit confirmed package/artifact evidence unchanged plus tag/release/two attestations absent.

TASK-0246 through TASK-0248 were a separate authorized continuation. TASK-0246 resolved/invoked `pwsh`, rejected Windows PowerShell regression, verified safety/artifact/state gates before mutation, and passed standard runs `29182095416`, `29182095415`, and three-OS source run `29182095423`. TASK-0247 then dispatched exact recovery once in run `29182188201`. Safety and exact artifact/package/signature/install verification passed, but the step exited after its expected absent-release probe and before mutation. One log read and one audit confirmed tag/release/two attestations absent. TASK-0248 success-only work was not executed. No normal publish, NuGet mutation, workflow rerun, second TASK-0247 dispatch, manual upload, tag movement, force update, or GA readiness claim is authorized.
...
Signing, SBOM, and provenance dispositions are recorded in [SUPPLY_CHAIN_DECISIONS.md](SUPPLY_CHAIN_DECISIONS.md). `scripts/test-supply-chain-workflow.ps1` verifies positive and negative permission/action boundaries through a resolved `pwsh` child. RC1 provenance remains absent because TASK-0247 stopped before tag/release/attestation. The TASK-0247 budget is consumed; any correction or future recovery needs a new explicit decision and must not republish NuGet.
