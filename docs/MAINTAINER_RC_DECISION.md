...
| NuGet ownership/signing | Published package has a valid NuGet.org repository signature, no observed author signature, and owner profile `Cyranth` versus project persona `Cynrath`; recovery decision owner is `Cynrath` but execution authority is unverified | Align/accept owner identity, verify recovery authority, and record explicit author sign/defer decision | Maintainer NuGet/supply-chain decision |
