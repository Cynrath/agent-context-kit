...
### V100 `1.0.0-rc.1` Candidate Input (TASK-0239 — Not Dispatched)

TASK-0239 prepares, but does not dispatch, the one authorized manual hosted RC evidence input:

| Input | Dispatch-time value |
| --- | --- |
| `commit_sha` | Exact TASK-0239 candidate commit, equal to local `HEAD` and `origin/master` after the candidate push |
| `candidate_version` | `1.0.0-rc.1` |
| `predecessor_version` | `0.2.0-alpha.4` |
| Exact predecessor config | Generated by installed alpha4 and frozen at `tests/fixtures/upgrade/v0.2.0-alpha.4-config.yml` |

The workflow will build a run-unique local package such as `1.0.0-rc.1.ci.<run-id>`; it must not publish or mutate the immutable `0.2.0-alpha.4` package. Before TASK-0240 dispatch, the exact candidate commit must already have green standard CI. TASK-0240 and TASK-0241 successors must remain docs/evidence/governance-only.
...
Single authorized TASK-0240 dispatch — do not run more than once for the exact SHA:
...
- exact predecessor config generation/fixture match, config hash/`config-check`, baseline, JSON/SARIF, localization/contract-gate, and clean-scan results;
...
Status: `CANDIDATE_INPUT_PREPARED`, `NOT_DISPATCHED`, `OPEN_PENDING_SINGLE_AUTHORIZED_WORKFLOW_DISPATCH`, `PUBLICATION_NOT_AUTHORIZED`.
