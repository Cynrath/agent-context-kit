...
TASK-0252 records a new owner-created exact-tag recovery boundary. The authenticated owner created `v1.0.0-rc.1` at `258918b33c3d1359aac967604ee524e8b66ddf02`; the workflow now verifies that existing tag twice and never creates, pushes, moves, deletes, or calls the refs API for it. GitHub prerelease/assets and both candidate-digest attestations were required to remain absent before the single separately authorized TASK-0253 dispatch. TASK-0252 performed no release or NuGet mutation.

TASK-0252 passed standard CI at commit `5f6c4ce2d0ab9745207196e6b01371653adfe009`. TASK-0253 then dispatched `recover-existing` exactly once as run `29345313517`. All immutable safety and artifact/package/tag/release-state gates passed, but `gh release create` returned HTTP 403. One log read and one remote audit confirmed package/artifact/tag unchanged and prerelease/assets/two attestations absent. The dispatch is consumed, TASK-0254 is not executed, and no rerun, second dispatch, manual release completion, settings change, or tag mutation is authorized.
