...
## Decision
Ship exactly one npm package (`agent-context-kit`, fallback per ADR-0013). Internal modularity via folders under `src/core/*`, not separate packages. No `@ackit/core|cli|mcp` splits until a real external consumer need is documented.
