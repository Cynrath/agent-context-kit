...
## Current Target
Maintain and verify the completed `v0.2.0-alpha.3` public pre-release while keeping future version, security, signing, SBOM, provenance, and recovery decisions maintainer-reviewed.
...
- `v0.2.0-alpha.3` tag is pushed at `92984c6448332aa24b7cff94647f627bf944e535`.
...
- GitHub Release page is completed for `v0.2.0-alpha.3` as a pre-release.
- NuGet publish is completed for `AgentContextKit` version `0.2.0-alpha.3`.
- NuGet global tool install and package smoke are verified for `0.2.0-alpha.3`.
...
- `v0.2.0-alpha.3` is published on GitHub and NuGet; `v0.2.0-alpha.2` remains the previous release.
- Codex for OSS form submission is completed per maintainer-provided status.
- Documentation-only published-package smoke examples install `AgentContextKit` `0.2.0-alpha.3`.
...
- The published NuGet `0.2.0-alpha.3` package includes `ackit sarif`; `0.2.0-alpha.2` remains the previous release.
- The published `0.2.0-alpha.3` package includes SARIF output, scanner rule catalog hardening, additive JSON `ruleId`, config-driven non-Critical allowlists, expanded scanner patterns, sample gallery docs, demo scenarios, Web UI preview docs, visual asset guidance, MCP stdio, `ackit watch`, `diff`, and `trim`.
...
- Published `0.2.0-alpha.3` includes scanner rule catalog hardening, additive JSON `ruleId` metadata, config-driven non-Critical allowlists, expanded risk patterns, baseline-aware CI, config diagnostics, sanitized suppression audit output, MCP stdio, `ackit watch`, `diff`, and `trim`.
