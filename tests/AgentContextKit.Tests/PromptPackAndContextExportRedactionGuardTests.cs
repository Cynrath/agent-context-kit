...
public sealed class PromptPackAndContextExportRedactionGuardTests
{
    private const string SyntheticMarker = "ackit-redaction-guard-marker-9e3b5c81";

    [Fact]
    public void PromptPackDoesNotLeakRawSecretValues()
    {
        using var repo = TempRepository.Create();
        repo.Write("AGENTS.md", "# Agents");
        repo.Write("docs/tasks/TASK-0001-redact-guard.md", "# Demo\n\nCompleted.\n");
        repo.Write("config/insecure.txt", $"raw-token={SyntheticMarker}");
        ...
        var scan = new ScanResult(
            ...
            [
                new RiskFinding(
                    ...
                    RiskCategory.Secret,
                    "config/insecure.txt",
                    "Synthetic redaction guard secret",
                    SyntheticMarker)
            ],
            HasReadme: false,
            HasLicense: false,
        ...
        generator.Generate(repo.Path, ".ackit/prompt-packs/redact-guard.md", LanguageCode.English, scan);

        var content = File.ReadAllText(Path.Combine(repo.Path, ".ackit", "prompt-packs", "redact-guard.md"));

        Assert.DoesNotContain(SyntheticMarker, content, StringComparison.Ordinal);
    }

    [Fact]
    public void ContextExportManifestDoesNotLeakRawSecretValues()
    {
        using var repo = TempRepository.Create();
        repo.Write("AGENTS.md", "# Agents");
        repo.Write("docs/tasks/TASK-0001-redact-guard.md", "# Demo\n\nCompleted.\n");
        ...
        var spec = new ContextExportSpec(
            ...
            OutputPath: ".ackit/context-exports/redact-guard.json",
            ApprovalMode: "explicit-approve",
            Language: LanguageCode.English);

        exportGen.Generate(repo.Path, spec, sourceScan);

        var content = File.ReadAllText(Path.Combine(repo.Path, ".ackit", "context-exports", "redact-guard.json"));
