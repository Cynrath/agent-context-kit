...
describe("builtin SKILL↔CLI parity (v0.4.1 maintenance)", () => {
  ...
  it("forbids master-only helpers in v0.4.1 templates", async () => {
    // `task archive --completed` and `TASK-COMPLETED-IN-ACTIVE` shipped after
    // v0.4.0 on the minor line; v0.4.1 templates must never instruct users to
    // run them (every documented command must exist on the v0.4.1 CLI).
    const { skills } = await discoverBuiltinSkills();
    const forbidden = ["archive --completed", "TASK-COMPLETED-IN-ACTIVE"];
    for (const skill of skills) {
      for (const file of ["SKILL.md", ...skill.files.filter((f) => f.startsWith("references/"))]) {
        const raw = await readFile(path.join(skill.sourceDir, file), "utf8");
        for (const marker of forbidden) {
          expect(
            raw.includes(marker),
            `${skill.name}/${file}: master-only '${marker}' must not ship in v0.4.1`,
          ).toBe(false);
        }
      }
    }
