...
describe("built dist CLI contract", () => {
  ...
  it(
    "prints the package.json version via --version from the single source of truth",
    () => {
      requireBuiltCli();
  ...
  it(
    "keeps stdout pure JSON in --json mode and exits 0",
    () => {
      requireBuiltCli();
  ...
  it(
    "exits with code 2 on invalid usage from the real executable",
    () => {
      requireBuiltCli();
