...
describe("Browser Bridge protocol — security", () => {
  ...
  it("payload cap: context returns JSON within 512KB, Content-Length guarded", async () => {
    const res = await httpRequest(handle as BrowserBridgeHandle, "/v1/context?maxTokens=80000", {
      headers: { Authorization: `Bearer ${(handle as BrowserBridgeHandle).token}` },
    });
