...
TASK-0232 through TASK-0238 were created with `ackit task` before implementation. TASK-0232 completed the V100 decision reconciliation. TASK-0233 completed mixed-corpus/time/memory/interruption/unreadable-file evidence. TASK-0234 selected exact source-impacting evidence base `b1604ae1e73017521d28e5a83f328bb1347406b6`; no version/publication selected; later commits must remain docs/evidence/governance-only. Current task is TASK-0235. Execute the remaining chain continuously with one local commit per task and no intermediate push. After the final local audit, push once and wait for `ci`, `cross-platform-smoke`, and `cross-platform-source-smoke` through the prescribed single blocking sequence. Stop at manual hosted RC workflow dispatch; do not mutate versions, packages, tags, releases, workflows, repository/security/collaborator settings, secrets, advisories, or recovery state.
