...
TASK-0187 was inserted as a user-prioritized hotfix before TASK-0181 to audit all three visible `nuget-release` failed deployment entries. Planning commit `fa45c0b` created the task and queue update; implementation commit `7a0e1f8` hardens release verification without remote release/tag/NuGet/deployment mutation. Deployment `5047180313` maps to release #4 job `81200598792` and the Ubuntu `powershell` executable failure. Deployments `5047227343` and `5047239131` map to release #5 jobs `81201079722` and `81201198341`, both failing on published-package temp path resolution. Current source now uses lazy writable temp resolution in `verify-published-package.ps1` and `verify-existing-release.ps1`, exposes a non-network temp self-test, and rejects Windows-only `powershell` inside the Ubuntu publish job through `check-release-workflow.ps1`. Local validation passed with 295/295 tests, source scan/doctor, release workflow gate, release recovery fixture, tracked/untracked guard, `git diff --check`, and `verify-release.ps1`. Hosted verification is pending push.
