...
TASK-0239 through TASK-0241 were created with `ackit task` and fully planned before implementation. TASK-0239 is current and prepares the exact source/package candidate plus alpha4 predecessor/config evidence, read-only hosted contract gates, draft release records, dependency review, and local package/install evidence before one candidate push and standard-CI wait. TASK-0240 is planned for exactly one authorized manual hosted RC dispatch and actual evidence recording. TASK-0241 is planned for docs-only bridge review, per-gap acceptance, final local validation, a joint evidence/docs push, and final standard CI. Stop at the separate TASK-0242 publication boundary. No publication, release workflow, tag/GitHub Release, provenance, upload, settings/security/collaborator change, force push, or GA claim is authorized.

TASK-0239 local preparation checkpoint: dependency vulnerability/deprecation reviews are clean; build is 0 warnings/0 errors; 431/431 tests and Unicode guard pass; disposable RC1 nupkg/snupkg metadata/content/hygiene and isolated install pass; installed alpha4-to-RC config/hash/baseline/SARIF smoke passes. All ACKit/contract/readiness gates pass; the RC-gate benchmark is 5.691s/45.1 MiB, the standalone benchmark is 4.193s/44.6 MiB, and the 428-file Markdown audit is clean. Candidate commit/push and exact-SHA standard CI remain.
