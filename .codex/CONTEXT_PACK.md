...
TASK-0232 through TASK-0238 were created with `ackit task` before implementation. TASK-0232 completed decisions, TASK-0233 expanded resource evidence, TASK-0234 selected source base `b1604ae1e73017521d28e5a83f328bb1347406b6`, and TASK-0235 refreshed the conditional contract freeze with all focused gates green. No version/publication was selected; later commits remain docs/evidence/governance-only. Current task is TASK-0236. Execute the remaining chain continuously with one local commit per task and no intermediate push. After the final local audit, push once and wait for `ci`, `cross-platform-smoke`, and `cross-platform-source-smoke` through the prescribed single blocking sequence. Stop at manual hosted RC workflow dispatch; do not mutate versions, packages, tags, releases, workflows, repository/security/collaborator settings, secrets, advisories, or recovery state.
