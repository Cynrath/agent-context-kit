...
- TASK-0240 hosted evidence is complete locally after exactly one dispatch: run `29118452246` passed on Windows job `86447580477`, Ubuntu job `86447580502`, and macOS job `86447580508`; exact candidate/predecessor/package/config/baseline/JSON/SARIF/localization/resource/final-scan evidence passed; uploads and publication were absent. Create its docs commit without pushing.
- TASK-0241 is current: verify the docs-only evidence bridge, reconcile each V100 gap, run final validation, create a separate docs commit, push TASK-0240/0241 together, and wait for final standard CI.
