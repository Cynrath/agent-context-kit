...
- TASK-0178 is implemented and hosted-verified: implementation commit `336c3ee` adds Core MCP contracts/router, deterministic `ackit.scan` / `ackit.findings` / `ackit.context` / `ackit.health` tools, and a thin `ackit mcp --stdio <json-request>` CLI stub. Evidence commit `90928f1` passed hosted runs on 2026-06-18: `ci` 27767741917, `cross-platform-smoke` 27767741926, and `cross-platform-source-smoke` 27767741935. Local validation reached 283/283 tests green; source `scan --ci` exits 0 with existing `.remember` Medium findings only; doctor is 13/13 PASS. TASK-0179 WebUI no-build static polish is next.
...
If context is compacted, continue from this file. PROJECT-CONTROL-0109 is active; TASK-0178 is implemented and hosted-verified through pushed evidence commit `90928f1`. Continue TASK-0179 through TASK-0186 in order without asking. Run the requested validation gates before commit/push, and do not force-push, rewrite history, move tags, create GitHub releases, publish NuGet packages, create remotes, call remote LLM APIs, handle API keys, upload SARIF/content, create issues/labels/settings, or automatically redact without explicit maintainer instruction.
