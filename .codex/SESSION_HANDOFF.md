...
- TASK-0187 is implemented locally as a user-prioritized hotfix before TASK-0181 and awaits hosted verification for the artifact-retention follow-up. Planning commit `fa45c0b` and implementation commit `7a0e1f8` audit all three visible failed `nuget-release` deployments (`5047180313`, `5047227343`, `5047239131`) and harden the release verification path without remote mutation. Root causes: release #4 Ubuntu publish job used Windows-only `powershell`; release #5 repeated published-package temp path null failures; release #4 attempt 2 failed because artifact `7611911273` expired after one-day retention. Current source uses lazy writable temp resolution in `verify-published-package.ps1` and `verify-existing-release.ps1`, adds `-TempResolutionSelfTest`, rejects `powershell` inside the Ubuntu publish job, and requires at least 14 days of validated package artifact retention. Local validation reached 296/296 tests green; focused guard tests reached 3/3; source `scan --ci`, doctor, release workflow gate, release recovery fixture, tracked/untracked guard, `git diff --check`, and `verify-release.ps1` passed.
...
If context is compacted, continue from this file. PROJECT-CONTROL-0109 is active; TASK-0187 artifact-retention follow-up is locally validated and awaits commit/push/hosted verification. After TASK-0187 hosted sync, continue TASK-0181 through TASK-0186 in order without asking. Run the requested validation gates before commit/push, and do not force-push, rewrite history, move tags, create GitHub releases, publish NuGet packages, create remotes, call remote LLM APIs, handle API keys, upload SARIF/content, create issues/labels/settings, or automatically redact without explicit maintainer instruction.
