...
- TASK-0187 is implemented locally as a user-prioritized hotfix before TASK-0181. Planning commit `fa45c0b` and implementation commit `7a0e1f8` audit all three visible failed `nuget-release` deployments (`5047180313`, `5047227343`, `5047239131`) and harden the release verification path without remote mutation. Root causes: release #4 Ubuntu publish job used Windows-only `powershell`; release #5 repeated published-package temp path null failures. Current source uses lazy writable temp resolution in `verify-published-package.ps1` and `verify-existing-release.ps1`, adds `-TempResolutionSelfTest`, and extends `check-release-workflow.ps1` to reject `powershell` inside the Ubuntu publish job. Local validation reached 295/295 tests green; source `scan --ci`, doctor, release workflow gate, release recovery fixture, tracked/untracked guard, `git diff --check`, and `verify-release.ps1` passed. Hosted verification is pending push.
