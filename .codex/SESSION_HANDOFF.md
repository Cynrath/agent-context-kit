...
- TASK-0256 implementation commit `0a9abd0` passed standard runs `29349381415`, `29349381490`, and `29349381465`. First attestation run/job `29349599514`/`87142124518` passed safety and exact release/package verification, then failed pre-attestation because the accepted HTTP 404 left native `$LASTEXITCODE=1`; all attestation/final-recheck/matrix steps skipped. Audit confirmed both attestations absent and tag unchanged. The verified-404 path now resets only that native failure state and static regression requires it; corrected validation/push/retry remain.
