...
- TASK-0256 is locally implemented. `attest-existing` uses only Contents read plus OIDC/attestation write, verifies exact tag/release/body/two assets and NuGet equivalence before two conditional attestations, verifies both signer-workflow subjects, rechecks immutable state, and gates Windows/Ubuntu/macOS installed-package smoke. Network-free negative fixtures cover wrong identity/body/hash/size/digest/set and forbidden mutations. Local 431/431, ACKit, release/V100/supply-chain/Markdown/YAML/Unicode gates pass; implementation commit/push/hosted evidence remain pending.
