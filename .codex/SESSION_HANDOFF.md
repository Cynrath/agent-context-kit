...
- TASK-0198 is the current independent docs-only investigation. It records exact remaining maintainer evidence for `RB-003` (independent backup security owner and notification coverage) and `RB-008` (destructive NuGet recovery authority and backup recovery owner). It does not select a new project control, close either blocker, or authorize release/tag/NuGet/version/security-setting changes.
