...
7. Archive after final evidence: `ackit task archive <id>` moves the completed task doc from `docs/tasks/active/` to `docs/tasks/archive/` (lookups resolve both locations by ID). Do not leave completed work in `active/`; archived completed tasks are not open work.
