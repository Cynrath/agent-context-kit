...
export async function bridgeFetch(
  ...
  try {
    ...
    if (host !== "127.0.0.1" && host !== "localhost" && host !== "[::1]" && host !== "::1") {
      return { ok: false, error: { code: "BAD_TARGET", message: "bridge target must be loopback", status: 400 } };
    }
  } catch {
  ...
  try {
    ...
    if (!contentType.includes("application/json")) {
      return { ok: false, error: { code: "BAD_CONTENT_TYPE", message: `expected json, got ${contentType}`, status: response.status } };
    }
    const lengthHeader = response.headers.get("content-length");
    if (lengthHeader !== null) {
      const len = Number.parseInt(lengthHeader, 10);
      if (Number.isFinite(len) && len > PAYLOAD_LIMIT_BYTES) {
        return { ok: false, error: { code: "PAYLOAD_TOO_LARGE", message: "bridge payload exceeds client limit", status: 413 } };
      }
    }
    ...
    try {
      ...
      return { ok: false, error: { code: "BAD_JSON", message: "invalid json from bridge", status: response.status } };
    }
    ...
    if ((error as Error).name === "AbortError") {
      return { ok: false, error: { code: "ABORTED", message: "bridge request aborted", status: 0 } };
    }
...
export async function fetchContext(session: BridgeSession, params: { profile?: string; maxTokens?: number }, signal?: AbortSignal) {
